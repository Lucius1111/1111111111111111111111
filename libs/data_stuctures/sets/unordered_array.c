...
void unordered_array_set_deleteElement(unordered_array_set *set, int value) {
    if ((unordered_array_set_in(set, value) < set->size - 1))
        swap(&set->data[unordered_array_set_in(set, value)],
             &set->data[--set->size], sizeof(int));
    else if ((unordered_array_set_in(set, value) == set->size - 1))
        set->size--;
}
...
unordered_array_set unordered_array_set_union(
        unordered_array_set set1, unordered_array_set set2) {
    unordered_array_set unionSet = unordered_array_set_create
...
unordered_array_set unordered_array_set_intersection(
        unordered_array_set set1, unordered_array_set set2) {
    unordered_array_set intersectionSet = unordered_array_set_create
            (set1.size | set2.size);
    for (size_t i = 0; i < set1.size; i++)
...
unordered_array_set unordered_array_set_difference(
        unordered_array_set set1, unordered_array_set set2) {
    unordered_array_set differenceSet =
            unordered_array_set_create(set2.size);
    for (size_t i = 0; i < set1.size; i++)
        if (unordered_array_set_in(&set2, set1.data[i]) == set2.size)
